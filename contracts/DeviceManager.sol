...
contract DeviceHelper is DeviceBase {
   ...
   */
  function isDeviceAnEntity(uint256 _deviceId) public view returns (bool) {
    return
      devices[_deviceId].owner ==
      address(uint160(uint256(devices[_deviceId].identifier)));
  }
   ...
   * @return Boolean status.
   */
  function isValidMetadataMember(
    uint256 _deviceId,
    bytes32[] memory _proof,
    bytes32 _leaf
  ) public view returns (bool) {
    return
      MerkleProof.verifyProof(_proof, devices[_deviceId].metadataHash, _leaf);
  }
   ...
   * @return Boolean status.
   */
  function isValidFirmwareHash(uint256 _deviceId, bytes32 _firmwareHash)
    public
    view
    returns (bool)
  {
   ...
   * @return Boolean status.
   */
  function isValidEthMessage(
    uint256 _deviceId,
    bytes32 _messageHash,
    bytes memory _signature
  ) public view returns (bool) {
    return
      ECRecovery.recover(_messageHash, _signature) ==
      address(uint160(uint256(devices[_deviceId].identifier)));
  }
}
...
contract SignatureBase {
  ...
  mapping(uint256 => uint256) public deviceSignatureCount;
  ...
  event SignatureRevoked(uint256 indexed signatureId, uint256 indexed deviceId);

  /// @dev Modifier for ensuring that the device hasn't been signed already.
  modifier notSigned(uint256 _deviceId) {
   ...
   */
  function signDevice(uint256 _deviceId, uint256 _expiryTime)
    public
    returns (uint256)
  {
    ...
    uint256 signatureId = signatures.push(signature) - 1;
   ...
   */
  function revokeSignature(uint256 _signatureId) public {
    require(
      signatures[_signatureId].signer == msg.sender,
      'Only for creator of the signature'
    );
    require(
      signatures[_signatureId].revoked == false,
      "Signature mustn't be revoked already"
    );
...
contract SignatureHelper is SignatureBase {
   ...
   */
  function getActiveSignaturesForDevice(uint256 _deviceId)
    public
    view
    returns (uint256[] memory)
  {
    uint256[] memory signatureIds = new uint256[](
      deviceSignatureCount[_deviceId]
    );
    uint256 counter = 0;
    for (uint256 i = 0; i < signatures.length; i++) {
      if (
        signatures[i].deviceId == _deviceId && signatures[i].revoked == false
      ) {
...
contract DeviceUpdatable is DeviceHelper, SignatureHelper {
   ...
   */
  function updateIdentifier(uint256 _deviceId, bytes32 _newIdentifier)
    public
    onlyOwnerOf(_deviceId)
    notSigned(_deviceId)
  {
   ...
   */
  function updateMetadataHash(uint256 _deviceId, bytes32 _newMetadataHash)
    public
    onlyOwnerOf(_deviceId)
    notSigned(_deviceId)
  {
   ...
   */
  function updateFirmwareHash(uint256 _deviceId, bytes32 _newFirmwareHash)
    public
    onlyOwnerOf(_deviceId)
    notSigned(_deviceId)
  {
